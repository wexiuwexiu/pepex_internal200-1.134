...
namespace GUI
{
	...
	LRESULT __stdcall WndProc(HWND Wnd, UINT Msg, WPARAM wParam, LPARAM lParam)
	{
		switch (Msg)
		{
			...
			case WM_KEYUP:
				...
				if (wParam == VK_F3)
					Visuals::GlowESP = !Visuals::GlowESP;
				if (wParam == VK_F4)
					Visuals::LootESP = !Visuals::LootESP;
				if (wParam == VK_F5)
					Visuals::DrawFOV = !Visuals::DrawFOV;
				if (wParam == VK_F6)
					Visuals::DrawName = !Visuals::DrawName;
				if (wParam == VK_F8)
					Visuals::Developer = !Visuals::Developer;
				if (wParam == VK_UP) {
					Visuals::DistanceESP += 100;
					if (Visuals::DistanceESP > 800) {
						Visuals::DistanceESP = 100;
					}
				}								
				if (wParam == VK_DOWN) {
					Visuals::DistanceESP -= 100;
					if (Visuals::DistanceESP <= 100) {
						Visuals::DistanceESP = 800;
					}
				}							
				if (wParam == VK_RIGHT) {
					Visuals::DistanceLoot += 50;
					if (Visuals::DistanceLoot >= 300) {
						Visuals::DistanceLoot = 50;
					}
				}								
				if (wParam == VK_LEFT) {
					Visuals::DistanceLoot -= 50;
					if (Visuals::DistanceLoot <= 50) {
						Visuals::DistanceLoot = 300;
					}
				}		
				break;
	...
	void SliderFloat(const wchar_t* Name, float* Current, float Min, float Max)
	{
		...
		Render::FillRoundedRectangle({ CurWnd.Pos.x + 10.f, CurWnd.Pos.y }, { CurWnd.Size.x - 20.f, 16.f }, Render::Color(0, 255, 200), 8.f);
	...
	void CheckBox(const wchar_t* Name, bool& Switch) {
		...
		Render::FillRoundedRectangle({ CurWnd.Pos.x + 10.f, CurWnd.Pos.y }, { 30.f, 16.f }, Switch ? Render::Color(0, 255, 200) : Render::Color(255, 0, 0), 8.f);
